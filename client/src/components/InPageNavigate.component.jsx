    ...
    return (
        <section>
            <div className="relative mb-8 border-b border-gray-300 flex flex-nowrap overflow-x-auto">
                {
                    routes.map((route, i) => <button ref={ (inPageNavIndex === i) ? activeTabRef : null } key={ i } className={ `btn text-wrap text-sm capitalize ${ (inPageNavIndex === i) ? "text-blue-700" : "text-gray-400" } ${defaultHiddenRoutes?.includes(route) && "md:hidden"}`} onClick={(e) => changeNavState(e.target, i)} >{ route }</button>)
                }
